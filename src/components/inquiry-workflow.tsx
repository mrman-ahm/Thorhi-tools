  ...
  return <form className="inquiry-workflow" onSubmit={submit} noValidate>
    <div className="inquiry-main">
      <section className="inquiry-block" aria-labelledby="selected-products-title">
        ...
        {inquiry.items.length === 0 ? <div className="empty-state compact"><h3>Your inquiry is empty.</h3><p>Browse the catalogue or add an unlisted item with the name or code you know.</p><div className="button-row"><Link className="button primary" href="/products">Browse products</Link><button className="button secondary" type="button" onClick={() => setShowManual(true)}>Add unlisted item</button></div></div> : <div className="inquiry-items">{inquiry.items.map(item => <article className="inquiry-row" key={item.key}><div className="inquiry-thumb" aria-hidden="true">{item.manual ? "Manual" : "Image"}</div><div className="inquiry-item-info"><small>{item.manual ? "UNLISTED ITEM" : "CATALOGUE PRODUCT"}</small><h3>{item.name}</h3><code>{item.variantLabel ?? item.code}</code><label htmlFor={`note-${item.key}`}>Product-specific note</label><textarea id={`note-${item.key}`} value={item.note} onChange={event => inquiry.updateItem(item.key, { note: event.target.value })} placeholder="Optional product requirement" /></div><div className="quantity-control"><label htmlFor={`qty-${item.key}`}>Quantity</label><div><button type="button" aria-label={`Decrease quantity for ${item.name}`} onClick={() => inquiry.updateItem(item.key, { quantity: item.quantity - 1 })}>−</button><input id={`qty-${item.key}`} type="number" min="1" max="9999" value={item.quantity} onChange={event => inquiry.updateItem(item.key, { quantity: Number(event.target.value) || 1 })} /><button type="button" aria-label={`Increase quantity for ${item.name}`} onClick={() => inquiry.updateItem(item.key, { quantity: item.quantity + 1 })}>+</button></div><button className="remove-button" type="button" onClick={() => removeItem(item.key)}>Remove</button></div></article>)}</div>}
