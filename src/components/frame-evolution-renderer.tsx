...
type SheetDescriptor = Required<
  Pick<SpriteDescriptor, "src" | "cellWidth" | "cellHeight" | "columns" | "startFrame" | "endFrame">
>;
...
function selectMedia(manifest: MediaManifest, useMobile: boolean): MediaSelection | null {
  if (manifest.evolution) {
    return {
      kind: "sheets",
      variant: useMobile ? manifest.evolution.mobile : manifest.evolution.desktop
    };
  }
  ...
  if (manifest.sprites) {
    return {
      kind: "legacy",
      descriptor: useMobile ? manifest.sprites.mobile : manifest.sprites.desktop
    };
  }

  return manifest.sprite
    ? { kind: "legacy", descriptor: { src: manifest.sprite, ...LEGACY_SPRITE } }
    : null;
}

function sheetIndexForFrame(variant: EvolutionVariant, frame: number) {
  const index = variant.sheets.findIndex(
    sheet => frame >= sheet.startFrame && frame <= sheet.endFrame
  );
...
export function FrameEvolutionScene() {
  ...
  useEffect(() => {
    const sectionCandidate = sectionRef.current;
    const canvasCandidate = canvasRef.current;
    if (!sectionCandidate || !canvasCandidate) return;

    const section: HTMLElement = sectionCandidate;
    ...
    const loadSheet = (index: number, generation = mediaGeneration) => {
      const variant = variantRef.current;
      const descriptor = variant?.sheets[index];
      if (!variant || !descriptor) {
        return Promise.reject(new Error("Evolution sheet unavailable"));
      }
    ...
    function render() {
      ...
      if (variant) {
        ...
        if (!cached) {
          ...
          void loadSheet(sheetIndex).catch(() => {
            if (!disposed) setMediaState("error");
          });
          return;
        }
    ...
    const configureMedia = (nextManifest: MediaManifest) => {
      ...
      const selection = nextManifest.available
        ? selectMedia(nextManifest, mobileMedia.matches)
        : null;
      if (!selection) {
        setMediaState("error");
        return;
      ...
      if (selection.kind === "sheets") {
        ...
        void loadSheet(initialIndex, generation)
          .then(() => {
          ...
          .catch(() => {
            if (!disposed && generation === mediaGeneration) setMediaState("error");
          });
        return;
      }
  ...
  useEffect(() => {
    ...
    const scope = createScope({ root: sectionRef }).add(() => {
      ...
      animate(".frame-evolution-chapter-marker", {
        scaleY: (_, markerIndex) => markerIndex === activeChapter ? 1 : 0.24,
        opacity: (_, markerIndex) => markerIndex === activeChapter ? 1 : 0.35,
        duration: 420,
        ease: "out(4)"
      });
    });
    ...
    <div className="frame-evolution-sticky">
      ...
      <div className="frame-evolution-layout container">
        <div
          ...
          role="img"
          aria-label="Scroll-controlled evolution of cutting and surgical instruments"
        >
          <canvas ref={canvasRef} aria-hidden="true" />
          <div className="frame-evolution-fallback" aria-hidden="true">
            <span>EVOLUTION SEQUENCE</span><small>MEDIA LOADING</small>
          </div>
          <div className="frame-evolution-feather" aria-hidden="true" />
          <div className="frame-evolution-frame-readout" aria-hidden="true">
            <span>FRAME</span><b ref={readoutRef}>001</b><small>/ 260</small>
          </div>
        </div>

        <div className="frame-evolution-copy-stack" aria-live="polite">
          {EVOLUTION_CHAPTERS.map((chapter, chapterIndex) => <article
            className="frame-evolution-copy"
            data-chapter={chapterIndex}
            data-active={activeChapter === chapterIndex}
            key={chapter.id}
          >
            ...
            <small>
              FRAMES {String(chapter.startFrame).padStart(3, "0")}–{String(chapter.endFrame).padStart(3, "0")}
            </small>
          </article>)}
        </div>
      </div>
      ...
      <div className="frame-evolution-timeline container" aria-hidden="true">
        {EVOLUTION_CHAPTERS.map(chapter => <span
          className="frame-evolution-chapter-marker"
          key={chapter.id}
        ><i />{chapter.index}</span>)}
        <b><i /></b>
      </div>
    </div>
  </section>;
